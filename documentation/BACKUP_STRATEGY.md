...
* **Storage Decoupling:** Pods and data are decoupled.
...
Having a backup is only half the strategy; the ability to rapidly restore is equally critical. Below are the Standard Operating Procedures (SOPs) for restoring data.
...
**Step 1: Create Volume from Snapshot (AWS Console)**
Go to **EC2 > Snapshots**, select the target backup, and click **Create Volume from Snapshot**. Ensure it is created in the same Availability Zone as your EKS nodes. Copy the new Volume ID (e.g., `vol-0abcd123456789xyz`).

**Step 2: Create Static PV & PVC in Kubernetes**
Create a manifest file (`restore-pv.yaml`) to map the new AWS EBS volume directly into Kubernetes:
...
**Step 3: Apply and Rebind**
Apply the manifest and restart the StatefulSet pod. The pod will automatically attach to the restored data volume:

```bash
kubectl apply -f restore-pv.yaml
kubectl delete pod mongodb-0 
# K8s will spin up a new mongodb-0 pod, automatically attaching the restored EBS volume.
